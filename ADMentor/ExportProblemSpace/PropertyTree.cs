...
namespace ADMentor.ExportProblemSpace
{
    public sealed class PropertyTree
    {
        ...
        private static IImmutableSet<Tuple<String, Option<String>>> CollectDiagramProperties(PropertyTree pt, IEnumerable<int> diagramElementIds)
        {
            ...
            return (from eid in elementId
                    where diagramElementIds.Contains(eid)
                    select pt.Properties)
                    .GetOrElse(() =>
                    {
                        return pt.Children.Aggregate(
                            ImmutableHashSet.Create<Tuple<String, Option<String>>>(),
                            (props, t) => props.Union(CollectDiagramProperties(t, diagramElementIds)));
                    });
        }
