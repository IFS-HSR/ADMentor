...
## Known Limitations and Issues (ADMentor Version 1.1)
...
* "Establish Dependencies from Problem Space" may terminate EA under some non-reproducible circumstances. Until now, data loss in the affected project file due to this bug did not occur.

* The XML files generated by "Tailor Problem Space" may contain more information than visible in EA after importing the file. This could lead to leaked confidential information in some rare cases.Importing a tailored XMI into the project that was used to export the file can lead to unexpected results - imported diagrams contain all original elements, even the ones that have been filtered out.
